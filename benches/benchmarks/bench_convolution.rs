...
fn convolution_big() {
    ...
    let TensorData::Float(my_y) = NaiveProvider::conv(TensorData::Float(x), TensorData::Float(w), Some(TensorData::Float(b)), attrs).unwrap() else {panic!("Unexpected return type")};
